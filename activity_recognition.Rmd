...
Training set can be found [here](https://d396qusza40orc.cloudfront.net/predmachlearn/pml-training.csv). It contains `r dim(training)[1]` observations with `r dim(training)[2]` variations each. First seven variations which are **`r names(training)[1:7]`** are descriptive, so we can get rid of them.
